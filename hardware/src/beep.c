...
void Beep_Init(void)
{
	...
	gpio_initstruct.GPIO_Pin = beep.PIN;						//将初始化的Pin脚
	gpio_initstruct.GPIO_Speed = GPIO_Speed_50MHz;				//可承载的最大频率
	
	GPIO_Init(beep.PORT, &gpio_initstruct);							//初始化GPIO
...
void Beep_Set(_Bool status)
{
	...
	GPIO_WriteBit(beep.PORT, beep.PIN, status == BEEP_ON ? Bit_SET : Bit_RESET);		//如果status等于BEEP_ON，则返回Bit_SET，否则返回Bit_RESET
